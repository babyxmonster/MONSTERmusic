...
/joinassistant [Chat nama pengguna atau chat ID]
- Gabung Asisten ke grup.

/leaveassistant [Chat nama pengguna atau chat ID]
- Asisten akan meninggalkan grup tertentu.

/leavebot [Chat nama pengguna atau chat ID]
- Bot akan meninggalkan chat khusus.
    ...
    if await is_active_chat(CallbackQuery.message.chat.id):
        if db_mem[CallbackQuery.message.chat.id]["videoid"] == videoid:
            ...
            return await CallbackQuery.answer(
                f"Tersisa {dur_left} Keluar dalam {duration_min} Menit.",
                show_alert=True,
            )
        return await CallbackQuery.answer(f"Ga main njing.", show_alert=True)
    else:
        return await CallbackQuery.answer(
            f"OS/VCG ga aktif tolol", show_alert=True
        )
...
async def activevc(_, message: Message):
    ...
    if await is_active_chat(message.chat.id):
        mystic = await message.reply_text("Sabar anjing... Mendapatkan Antrian..")
        dur_left = db_mem[message.chat.id]["left"]
        duration_min = db_mem[message.chat.id]["total"]
        got_queue = get_queue.get(message.chat.id)
        if not got_queue:
            await mystic.edit(f"Ga ada antiran babi astaga")
        fetched = []
        ...
        msg = "**Daftar Antrian**\n\n"
        msg += "**Saat ini bermain:**"
        msg += "\n▶️" + current_playing[:30]
        msg += f"\n   ╚Dari:- {user_name}"
        msg += f"\n   ╚Durasi:- Tersisa `{dur_left}` keluar dalam `{duration_min}` Menit."
        fetched.pop(0)
        if fetched:
            msg += "\n\n"
        ...
        await message.reply_text(f"Ga ada antrian babi astaga")
...
async def activevc(_, message: Message):
    ...
    for x in served_chats:
        ...
        j += 1
    if not text:
        await message.reply_text("OS/VCG ga aktif tolol")
    else:
...
async def basffy(_, message):
    ...
    try:
        ...
        await message.reply_text(f"Gagal\n**kemungkinan alasan bisa**:{e}")
        return
    await message.reply_text("Join.")
...
async def baaaf(_, message):
    ...
    try:
        ...
        print(e)
        return
    await message.reply_text("Bot telah berhasil meninggalkan Grup")
...
async def baujaf(_, message):
    if len(message.command) != 2:
        await message.reply_text(
            "**Menggunakan:**\n/leave [Chat nama pengguna Atau chat ID]"
        )
        return
    chat = message.text.split(None, 2)[1]
    try:
