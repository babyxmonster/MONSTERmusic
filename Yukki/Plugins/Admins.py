...
/activevc
- Check Obrolan suara aktif di bot.
...
async def admins(_, message: Message):
    ...
    if not len(message.command) == 1:
        return await message.reply_text("ERROR! Salah Perintah lu cok.")
    if not await is_active_chat(message.chat.id):
        return await message.reply_text("Ga ada musik nya cok goblok.")
    chat_id = message.chat.id
    if message.command[0][1] == "a":
        if not await is_music_playing(message.chat.id):
            return await message.reply_text("Musiknya Lagi Di Jeda Cok.")
        await music_off(chat_id)
        await Yukki.pytgcalls.pause_stream(chat_id)
        await message.reply_text(
            f"🎧 Dijeda Sama Si {message.from_user.mention} Bocah Tolol!"
        )
    if message.command[0][1] == "e":
        if await is_music_playing(message.chat.id):
            return await message.reply_text("Alhamdulilah Dimulai Lagi.")
        await music_on(chat_id)
        await Yukki.pytgcalls.resume_stream(message.chat.id)
        await message.reply_text(
            f"🎧 Di lanjutin Sama {message.from_user.mention} Setan!"
        )
    if message.command[0][1] == "t" or message.command[0][1] == "n":
        ...
        await message.reply_text(
            f"🎧 DI STOP SAMA {message.from_user.mention} ANJING!"
        )
    if message.command[0][1] == "k":
        ...
        if Queues.is_empty(chat_id):
            await remove_active_chat(chat_id)
            await message.reply_text(
                "Tidak Ada __ANTRIAN__ Anjing \n\nMeninggalkan Voice Chat"
            )
            await Yukki.pytgcalls.leave_group_call(message.chat.id)
            return
        else:
            ...
            if str(finxx) != "raw":
                mystic = await message.reply_text(
                    f"**{MUSIC_BOT_NAME} Fungsi Daftar Putar**\n\n__LAGI CARI MUSIK NYA SABAR DULU COK ASU....__"
                )
                (
                    title,
                    duration_min,
                ...
                final_output = await message.reply_photo(
                    photo=thumb,
                    reply_markup=InlineKeyboardMarkup(buttons),
                    caption=f"<b>__ini Lagu selanjutnya ya cok__</b>\n\n🎥<b>__JUDUL:__</b> {title} \n⏳<b>__DURASI:__</b> {duration_min} \n👤<b>__Dimulai Oleh:__ </b> {mention}",
                )
            await start_timer(
                videoid,
                duration_min,
